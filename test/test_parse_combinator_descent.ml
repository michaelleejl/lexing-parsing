...
open Printf
open Token
open Descent (Grammar)

let print_ast x = Ast.to_str x |> printf "%s"

let%expect_test _ =
  print_ast (parse [ Token.FUN; IDENT "x"; ARROW; IDENT "x"; PLUS; NUM 0 ]);
  [%expect {| Fun(x, Plus(x, 0)) |}]

let%expect_test _ =
  print_ast (parse [ Token.NUM 1; PLUS; NUM 2; PLUS; NUM 3 ]);
...
let%expect_test _ =
  print_ast (parse [ Token.NUM 1; PLUS; NUM 2; EQUALS; NUM 3 ]);
  [%expect {| Equals(Plus(1, 2), 3) |}]

let%expect_test _ =
  print_ast (parse [ Token.IDENT "f"; LPAREN; NUM 1; PLUS; NUM 2; RPAREN ]);
