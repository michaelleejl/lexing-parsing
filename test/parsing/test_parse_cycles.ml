...
module Self = BottomUp_Elaborate (Cyclic_grammars.Self_cycle)
module Unit_ = BottomUp_Elaborate (Cyclic_grammars.Unit_cycle)
module Padded = BottomUp_Elaborate (Cyclic_grammars.Padded_cycle)
module Left_rec = BottomUp_Elaborate (Cyclic_grammars.Left_recursive)
module Null_tail = BottomUp_Elaborate (Cyclic_grammars.Nullable_tail)
module Null_prefix = BottomUp_Elaborate (Cyclic_grammars.Nullable_prefix)
module Natural = BottomUp_Elaborate (Grammars.Natural)
module Left_factored = BottomUp_Elaborate (Grammars.LeftFactored)
module Ll1 = BottomUp_Elaborate (Grammars.LL1)
...
let%expect_test "shipped grammars are acyclic" =
  case "Natural" "acyclic" (fun () ->
      ...
      ignore M.Cycles.table);
  case "LeftFactored" "acyclic" (fun () ->
      let module M = GrammarAnalysis (Left_factored.Bnf) in
      ignore M.Cycles.table);
