...
module Report (Gram : Intfs.Grammar.S) = struct
  ...
  let nt = Gram.Nonterminal.to_string
  let term = Gram.Terminal.to_string
  let sym = Gram.to_string
  ...
  let productions =
    ...
    :: List.filter_map
         (function
           | Gram.Production { lhs; rhss } -> Some (lhs, rhss)
           | Gram.Consumption _ -> None)
         Gram.grammar
