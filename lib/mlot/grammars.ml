...
module Natural = struct
  ...
  module Data = struct
    type t = Nothing | Num of int | Name of string | Expr of ast
    [@@deriving compare]
  end
  ...
  let mk_letrec x e1 e2 = Expr (Let_rec (x, e1, e2))
  let mk_var x = Expr (Var x)
  let mk_num n = Expr (Num n)
  let mk_bool b = Expr (Bool b)
  let mk_eq e1 e2 = Expr (Equals (e1, e2))
  let mk_plus e1 e2 = Expr (Plus (e1, e2))
  let mk_app e1 e2 = Expr (App (e1, e2))

  module Nonterminal = struct
    type t = E | T | F | G | S [@@deriving compare, to_string]
  end
  ...
  let prod_t_equals =
    {
      lhs = Nonterminal.T;
      rhs = [ N T; T EQUALS; N F ];
      builder = [%act function [ Expr e1; _; Expr e2 ] -> mk_eq e1 e2];
    }
  ...
  let prod_t = [ prod_t_equals; prod_t_f ]
  ...
  let prod_f_plus =
    {
      lhs = F;
      rhs = [ N F; T PLUS; N G ];
      builder = [%act function [ Expr e1; _; Expr e2 ] -> mk_plus e1 e2];
    }
  ...
  let prod_f = [ prod_f_plus; prod_f_g ]
  ...
  let prod_g_app =
    {
      lhs = G;
      rhs = [ N G; N S ];
      builder = [%act function [ Expr e1; Expr e2 ] -> mk_app e1 e2];
    }
  ...
  let prod_g = [ prod_g_app; prod_g_s ]
  ...
  let prod_s_true =
    {
      lhs = S;
      rhs = [ T TRUE ];
      builder = [%act function [ _ ] -> mk_bool true];
    }
  ...
  let prod_s_false =
    {
      lhs = S;
      rhs = [ T FALSE ];
      builder = [%act function [ _ ] -> mk_bool false];
    }
  ...
  let cons_silent tok : consumption =
    ...
    {
      lhs = term;
      reader = (function t when t = tok -> Nothing | _ -> raise Fail);
    }
  ...
  let productions = List.concat [ prod_e; prod_t; prod_f; prod_g; prod_s ]
