...
module LL1 (Gram : Grammar.S) = struct
  ...
  open Analysis.Lookahead(Gram)

  type parser = token list -> data * token list 
  type predictive_parser = {
    guard: TSet.t;
    parser: parser
  }

  let predict {guard=g1; parser=p1} 
          {guard=g2; parser=p2} = 
      if TSet.disjoint g1 g2 then 
        let parser = function 
          | tok::_ as toks -> 
            let term = Gram.token_to_terminal tok in 
            if TSet.mem term g1 then 
              p1 toks 
           else if TSet.mem term g2 then 
              p2 toks 
           else 
              raise (ParseFail "unexpected token")
          | _ -> raise (ParseFail "bad parse")
        in 
        {guard=TSet.union g1 g2; 
         parser }
      else 
        raise (ParseFail "grammar not in LL1")
  ...
  let production_to_parser fs lhs { rhs; action }  =
    let parser toks = 
      let accumulator, toks' = production_to_accumulator fs rhs toks in
      (action accumulator, toks') in 
    let guard = 
      let first = unwrap (first_syms rhs) in 
      if nullable_syms rhs then 
        TSet.union (first) (follow lhs)
      else
        first
      in 
    {parser; guard}
  ...
  let productions_to_parsers ((lhs, pss) : nonterminal * production list) fs =
    let {parser} = List.fold_left ( >>| ) empty (List.map (production_to_parser fs lhs) pss) in 
    parser
