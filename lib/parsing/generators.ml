...
struct
  ...
  module ParseStack = struct 
    ...
    let create_frame remaining action = {
        remaining;
        collected = [];
        action;
    }
  ...
  module ParseHypothesis = struct 
    type t = TaggedPda.config * ParseStack.t
        [@@deriving compare]
  end 
  ...
  let advance_one machine token ((config, stack): ParseHypothesis.t) = 
    let traces = TaggedPda.consume machine (TaggedPda.TraceSet.singleton (config, [])) (Gram.token_to_terminal token) in 
    TaggedPda.TraceSet.fold (collect_traces token stack) traces 
    ParseHypotheses.empty 
  ...
  let rec parse_run machine {tokens ; hypotheses } = 
    ...
    | [] ->  
        let accepting = ParseHypotheses.filter 
            (fun (cfg, _) -> TaggedPda.is_accepting cfg) hypotheses in
  ...
  module Transition = struct 
    type t = (terminal option * Gram.t) * (TaggedPda.state * Gram.t list * ParseTag.t)
    [@@deriving compare]
  end 
  ...
  let rule_to_transition state = function 
    | Production {lhs; rhss} -> 
        let ts = List.map (fun {rhs; action} ->(nonterminal_to_transition lhs rhs action state)) rhss in 
  ...
  let compile grammar = 
    let state = 0 in 
    let ts = 
        List.fold_right (>>|) (List.map (rule_to_transition state) grammar) TransitionSet.empty in 
    let transitions = TransitionSet.fold
     (fun (input, output) -> fun transitions ->
        let existing = match TaggedPda.Transition.find_opt input transitions with
          | Some outputs -> outputs
          | None -> TaggedPda.TransitionOutputSet.empty
        in
        TaggedPda.Transition.add input
          (TaggedPda.TransitionOutputSet.add output existing) transitions
      )
     ts TaggedPda.Transition.empty in
    TaggedPda.{
    ...
    let parse tokens = 
        let initial_hypothesis =
            ( TaggedPda.Config.{ current_state = parser.TaggedPda.initial_state;
